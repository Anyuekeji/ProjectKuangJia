...
- (void) _setUpICarousel {
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-8-[iCarousel]-6-|" options:0 metrics:nil views:_binds]];
}

- (void) _setUpPageControl {
    _pageControl = [[UIPageControl alloc] init];
    self.pageControl.pageIndicatorTintColor = UIColorFromRGBA(0xffc7cf,0.5);
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-12-[pageControl]-12-|" options:0 metrics:nil views:_binds]];
    _pageControlBottomConstraint = [NSLayoutConstraint constraintWithItem:self.pageControl attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeBottom multiplier:1.0f constant:0.0f];
...
- (CATransform3D)carousel:(iCarousel *)carousel itemTransformForOffset:(CGFloat)offset baseTransform:(CATransform3D)transform {
    ...
    return CATransform3DTranslate(transform, offset * self.iCarouselView.itemWidth * 1.28, 0.0, 0.0);
}
