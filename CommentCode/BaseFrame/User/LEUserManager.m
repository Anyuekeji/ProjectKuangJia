...
@interface LEUserManager ()
@property (nonatomic, readonly, strong) LEMeModel * userItem;
@end
@implementation LEUserManager
+ (LEUserManager *) sharedInstance {
...
- (void) loadUserItem {
    if ( [LEFileManager isFileExistsInDocuments:kAYUserCacheFileName] ) {
        _userItem = [LEMeModel loadFromDocumentWithFileName:kAYUserCacheFileName];
    }
}
...
- (BOOL) setUserItemByRecord : (NSDictionary *) record {
    ...
    _userItem = [LEMeModel itemWithDictionary:record];
    
    return [self saveUserItem];
}

- (BOOL) setUserItemByItem: (LEMeModel *) model {
